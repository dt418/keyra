import { Card, CardContent, CardHeader, CardTitle, CardDescription, PageHeader, Button, StatusBadge, Tabs, TabsList, TabsTrigger, TabsContent } from '@/components/ui';
import { Book, Code2, Key, Copy, Check, Package, Terminal, Zap } from 'lucide-react';
import { toast } from 'sonner';
import { useState } from 'react';

const installCode = {
  npm: 'npm install @keyra/sdk-js',
  pnpm: 'pnpm add @keyra/sdk-js',
  yarn: 'yarn add @keyra/sdk-js',
  bun: 'bun add @keyra/sdk-js',
};

const initCode = `import { createClient } from '@keyra/sdk-js';
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2">
            {[
              { method: 'POST', endpoint: '/api/v1/auth/register', desc: 'Register a new user' },
              { method: 'POST', endpoint: '/api/v1/auth/login', desc: 'Sign in and get tokens' },
              { method: 'POST', endpoint: '/api/v1/products', desc: 'Create a new product' },
              { method: 'GET', endpoint: '/api/v1/products', desc: 'List products' },
              { method: 'POST', endpoint: '/api/v1/verify', desc: 'Verify a license key' },
              { method: 'POST', endpoint: '/api/v1/activate', desc: 'Activate a license on a device' },
              { method: 'DELETE', endpoint: '/api/v1/devices/:id', desc: 'Deactivate a device' },
              { method: 'POST', endpoint: '/api/v1/licenses/:id/revoke', desc: 'Revoke a license' },
            ].map((route) => (
              <div key={route.endpoint} className="flex items-center gap-3 rounded-md border border-border p-3 hover:bg-accent transition-colors">
                <span className={`rounded px-2 py-0.5 text-xs font-semibold ${
                  route.method === 'GET' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' :
                  route.method === 'POST' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :
                  route.method === 'DELETE' ? 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400' :
