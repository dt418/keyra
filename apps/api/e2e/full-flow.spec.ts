...
test.describe("Full License Flow", () => {
  test("user can register, create org, create product, generate license, activate device, verify, deactivate", async ({
    request,
  }) => {
    ...
    expect(
      listBody.data.some((l: { id: string }) => l.id === licenseBody.data.id),
    ).toBe(true);
    ...
    const revoke = await request.post(
      `licenses/${licenseBody.data.id}/revoke`,
      {
        headers: auth,
  ...
  test("404 for non-existent product", async ({ request }) => {
    const reg = await request.post("auth/register", {
      data: {
        email: `e2e-404-${Date.now()}@example.com`,
