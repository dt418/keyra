...
describe('listProductsHandler', () => {
  ...
  it('should list products for admin user', async () => {
    ...
    mockDB.all.mockResolvedValueOnce([
      { id: 'prod-1', name: 'Product 1', description: 'Desc 1', created_at: '2024-01-01', updated_at: '2024-01-01' },
      { id: 'prod-2', name: 'Product 2', description: null, created_at: '2024-01-02', updated_at: '2024-01-02' },
    ]);
  ...
  it('should throw if user not authenticated', async () => {
    ...
    await expect(listProductsHandler(ctx)).rejects.toThrow('Authentication required');
  });
...
describe('createProductHandler', () => {
  ...
  it('should create product successfully', async () => {
    ...
    const ctx = createMockContext({
      req: {
        json: vi.fn().mockResolvedValue({ name: 'Test Product', description: 'Test desc' }),
        query: vi.fn().mockReturnValue({}),
        param: vi.fn().mockReturnValue({}),
        header: vi.fn().mockReturnValue('Bearer token'),
      },
  ...
  it('should reject if user is not admin', async () => {
    ...
    await expect(createProductHandler(ctx)).rejects.toThrow('Admin or owner role required');
  });
});
...
describe('updateProductHandler', () => {
  ...
  it('should update product successfully', async () => {
    ...
    const ctx = createMockContext({
      req: {
        json: vi.fn().mockResolvedValue({ name: 'Updated', description: 'New desc' }),
        query: vi.fn().mockReturnValue({}),
        param: vi.fn().mockReturnValue({ id: 'prod-1' }),
        header: vi.fn().mockReturnValue('Bearer token'),
      },
...
describe('deleteProductHandler', () => {
  ...
  it('should return 403 for non-owner', async () => {
    ...
    await expect(deleteProductHandler(ctx)).rejects.toThrow('Only owners can delete products');
  });
});
