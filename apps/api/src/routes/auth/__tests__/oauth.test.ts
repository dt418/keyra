...
function createMockContext(overrides: Record<string, unknown> = {}) {
  ...
  return {
    ...
    req: {
      json:
        (reqOverrides.json as typeof vi.fn) || vi.fn().mockResolvedValue({}),
      param:
        (reqOverrides.param as typeof vi.fn) || vi.fn().mockReturnValue({}),
      header: (name?: string) => {
...
describe("oauthCallbackHandler", () => {
  ...
  it("should return tokens for new user (google)", async () => {
    const ctx = createMockContext({
      req: {
        ...
        json: vi
          .fn()
          .mockResolvedValue({ code: "valid-code", state: "valid-state" }),
      },
    }) as any;
    ...
    fetchMock.mockResolvedValueOnce(
      new Response(
        new URLSearchParams({ access_token: "provider-token" }).toString(),
        { status: 200 },
      ),
    );
    fetchMock.mockResolvedValueOnce(
  ...
  it("should return 400 for invalid state (CSRF)", async () => {
    const ctx = createMockContext({
      req: {
        ...
        json: vi
          .fn()
          .mockResolvedValue({ code: "valid-code", state: "invalid-state" }),
      },
    }) as any;

    mockKV.get.mockResolvedValueOnce(null);

    await expect(oauthCallbackHandler(ctx)).rejects.toThrow(
  ...
  it("should return 400 for invalid code", async () => {
    const ctx = createMockContext({
      req: {
        ...
        json: vi
          .fn()
          .mockResolvedValue({ code: "invalid-code", state: "valid-state" }),
      },
    }) as any;
    ...
    fetchMock.mockResolvedValue(
      new Response("error=bad_verification_code", { status: 400 }),
    );
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("should link OAuth to existing email account", async () => {
    ...
    mockKV.get.mockResolvedValueOnce("google");
    mockDB.first.mockResolvedValueOnce(null).mockResolvedValueOnce({
      id: "linked-user",
  ...
  it("should reject account takeover: existing user linked to different provider", async () => {
    ...
    await expect(oauthCallbackHandler(ctx)).rejects.toThrow(
      "This email is linked to github",
    );
  });
